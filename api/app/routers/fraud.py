...
@router.post("", status_code=201, dependencies=[Depends(verify_feature("fraud_detection"))])
...
@router.get("/{check_id}", dependencies=[Depends(verify_feature("fraud_detection"))])
...
@router.get("", dependencies=[Depends(verify_feature("fraud_detection"))])
